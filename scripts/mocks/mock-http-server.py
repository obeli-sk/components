...
Every request must carry a Host header. A client that omits it (as the
obelisk 0.41.3 HTTP host did) is rejected with 400 so the regression fails
loudly instead of surfacing as a confusing connection reset.
"""
...
class MockHTTPHandler(BaseHTTPRequestHandler):
    # HTTP/1.1 so chunked request bodies are parsed; every response sets
    # Connection: close, so each request uses its own connection.
    ...
    def require_host(self):
        """Reject requests without a Host header (HTTP/1.1 requires one)."""
